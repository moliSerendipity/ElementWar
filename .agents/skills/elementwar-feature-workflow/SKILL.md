---
name: elementwar-feature-workflow
description: 在 ElementWar 仓库中处理新增功能、跨文件重构、架构或公共接口、序列化数据、性能、兼容性、迁移或复杂缺陷时，执行只读检查、需求澄清、明确授权、垂直切片实施、Unity 验证和证据化交付；知识问答、只读审查、状态报告和明确的低风险小改动不触发。
---

# ElementWar 功能工作流

把大型任务转化为有批准行为契约、可验证切片和真实证据的交付，同时保护用户未提交改动。

## 加载所需上下文

1. 完整遵守项目根 `AGENTS.md`。
2. 对本 skill 触发的任务读取 `Docs/Workflow.md`。
3. 用户说“继续下一阶段”、询问开发顺序或没有点名具体功能时，读取 `Docs/DevelopmentRoadmap.md`，再只读取其中唯一 `Next` 指向的路线细则。用户明确指定功能时只核对该功能的路线任务和依赖。
4. 仅在涉及程序集、状态所有权、scene/prefab、序列化、旧架构或迁移时读取 `Docs/Architecture.md`。
5. 仅在新增、修改或审查 C# 时读取 `Docs/CodingStandards.md`。
6. 仅在设计或执行测试、构建、性能检查时读取 `references/verification-matrix.md`。
7. 读取相关实现、asmdef、配置、场景、测试、文档、Git 状态和 scoped diff；不要预先加载无关文档。
8. 涉及玩家可见玩法、战斗、关卡流程或网络规划时，先用 `Docs/GameDesign.md` 定位，再按需读取一份或少量细则：
   - 角色、AI、移动、武器、弹药、技能、伤害、输入或 HUD：`Docs/Design/Combat.md`
   - 元素附着、反应、控制或元素扩展：`Docs/Design/Elements.md`
   - 场景、波次、敌人编排、Boss、补给或肉鸽强化：`Docs/Design/Run.md`
   - 联机边界、网络实验或同步约束：`Docs/Design/Networking.md`
9. 不得默认加载全部设计细则；文档拆分和唯一事实源规则以 `Docs/Workflow.md` 为准。

如果这些文件仍位于候选包而未落到真实项目根，只把它们视为提案，不得声称对应规则、skill 或命令已经在实时工程生效。

## 执行流程

### 0. 选择适合当前风险的任务结构

- 先按 `Docs/Workflow.md` 判断是局部闭环、标准垂直切片、高风险迁移还是探索任务，不预设所有工作必须合并或拆分。
- 路线任务只定义依赖、行为和证据边界；Feature Spec 承载本次批准的具体切片。发现路线缺项或依赖错误时先修正路线提案，不绕过前置任务。
- 把仓库和权威文档能够回答的内容一次性自行确认；只集中询问剩余的高影响选择。

### 1. 建立事实

- 找到实际入口、调用链和权威状态所有者。
- 列出配置结构、请求输入和可变运行时状态各自的不变量与唯一校验层；发现同一条件跨层重复时先判断是否存在新的失败来源。
- 检查已有实现、序列化引用、测试入口和用户未提交改动。
- 分开记录已验证事实、推断、待用户决定和可能过期的文档。
- 不询问仓库能够回答的问题。

### 2. 建立 Feature Spec

使用 `Docs/Features/TEMPLATE.md`，明确目标、非目标、范围、设计、获批复杂度边界、校验所有权、运行时热路径、生命周期、验收、验证、风险和回滚。复杂度边界应列出预计新增的脚本、类型、配置资产和公共接口，以及每项当前责任或已批准扩展依据。给出推荐方案及取舍，但把高影响决策留给用户。

### 3. 等待授权

总结目标、范围、方案、假设、风险和验收标准。获得“开始实施”“按此执行”或同等明确授权后才能编辑或改变项目状态。实施中出现新的高影响决策时再次暂停。

### 4. 实施切片

- 一次完成一个最小端到端行为。
- 复用当前权威状态和已发生事实事件。
- 处理适用的初始化、启用/禁用、订阅/退订、死亡、重置和对象池复用。
- 每项不变量只在其权威层校验；Gameplay 热路径不得重复完整配置校验。跨层防御必须对应上游之后可能发生的真实失败，并写明原因。
- 新增或修改 C# 的多阶段方法时，按逻辑块补充必要中文注释，解释目的、顺序和不变量；不逐句翻译，也不留下需要读者反推规则的大段实现。
- 不得静默超出获批复杂度边界；需要新增脚本、类型、配置资产、公共接口或把固定规则配置化时，先说明真实消费者、现有方案无法表达的行为和最小替代方案，并重新获得授权。
- 不做无关清理、广泛格式化、旧架构扩展或未经批准的通用化。
- 每个切片后检查 scoped diff。

### 5. 验证与审查

- 从最精确、最低成本的检查开始，再按 `Docs/Workflow.md` 提升验证等级。
- 测试数为 0 必须失败；不得删弱测试只为得到绿色结果。
- 保存原始 XML、日志、Player、截图/录像或 Profiler 证据。
- scoped review 搜索本次调用链中的 `Validate`、`HasValid`、`IsValid`、`Enum.IsDefined` 等校验，确认没有把同一不变量重复放入启动期与每次命中的热路径；同时检查触及的多阶段函数是否有准确的逻辑块注释。
- 按 `Docs/Workflow.md` 执行必要性与扩展性审查，不把“能否删除”作为唯一标准：保留承担当前职责的实现，以及已批准需求所需、延后会产生已知迁移成本且属于最低成本稳定接缝的扩展点；简化或移除仅为假设未来存在并引入未使用状态、分支、资产、缓存或重复校验的抽象。
- 根据公共契约、状态所有权、序列化、物理/计时、迁移、发布影响、测试可信度和 diff 复杂度决定是否需要独立只读审查，不把它作为每个切片的固定步骤。
- 审查结论按“已确认缺陷、违反验收、证据缺口、可选增强”分类；只有前两类默认要求修复后重新验证，证据缺口先修正完成声明或补证据，可选增强进入路线或后续任务。

### 6. 证据化交付

报告实际行为和文件、实际命令、测试数量与结果、证据路径、人工验收、未运行项、剩余风险、回滚方式和文档同步情况。严格区分 Implemented、Fast Verified、Full Verified 和 Accepted。功能属于开发路线时，同步任务状态、证据链接、解锁项和新的唯一 `Next`。

## 必须暂停的情况

- 新选择会改变行为、公共 API、序列化、架构、迁移或验收。
- 必需修改与用户改动重叠且无法安全保留。
- 实时工程与批准后的 Feature Spec 冲突。
- 需要破坏性迁移或扩大平台范围。
- Unity 验证需要用户关闭 Editor 或提供外部条件。
