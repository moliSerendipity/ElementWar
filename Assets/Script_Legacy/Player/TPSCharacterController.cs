...
public class TPSCharacterController : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        // 反向推送模式：C# 检测到了有效意图，推送给 Lua 进行决策审核
        if (inputFrame.reloadButton.wasPressedThisFrame)
            LuaManager.Instance.NotifyReloadInput(characterID);

        if (inputFrame.switchAmmoButton.wasPressedThisFrame)
            LuaManager.Instance.NotifySwitchAmmoInput(characterID);

        if (inputFrame.fireButton.isHeld || inputFrame.fireButton.wasPressedThisFrame)
            LuaManager.Instance.NotifyFireInput(characterID, inputFrame.fireButton.isHeld, inputFrame.fireButton.wasPressedThisFrame);
    }
    ...
    public void SetPlayerControl(bool _isPlayerControlled)
    {
        if (_isPlayerControlled)
        {
            ...
            // 切人为玩家时，必须强制认领一次主摄像机，否则新切出来的角色旋转参照系为 null
            if (Camera.main != null)
                MainCameraTransform = Camera.main.transform;
        }
        else
    ...
    /// <summary>
    /// 被 Lua 调用，表示开火审核（CD、弹药量）通过，生成实战开火效果
    /// </summary>
    public void ForceFireHitscan(int _ammoConfigID, int _elementType)
    {
        Animator.TriggerFire();
        if (WeaponController != null)
            WeaponController.FireHitscan(_ammoConfigID, characterID, _elementType, 150f); // 默认距离 150 米
    }

    /// <summary>
    /// 被 Lua 调用：武器换了，需要同步手中真实的3D模型与IK抓取手位
    /// </summary>
    public void ChangeWeaponModel(GameObject _weaponPrefab)
    {
        if (WeaponController != null)
            WeaponController.EquipWeaponModel(_weaponPrefab);
    }
