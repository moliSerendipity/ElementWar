...
public class LuaManager : SingleMonoBase<LuaManager>
{
    ...
    [CSharpCallLua]
    public delegate void LuaUpdateDelegate(float _deltaTime);
    private LuaUpdateDelegate luaGameUpdate;

    [CSharpCallLua]
    public delegate void OnHitMessageDelegate(int _instigatorID, int _targetUID, int _targetConfigID, int _targetLevel, int _ammoID, float _hitMultiplier);
    private OnHitMessageDelegate luaOnHitMessage;

    // 用于 C# 意图推送到 Lua 的输入映射
    [CSharpCallLua] public delegate void NotifyInput_Bool1(int _characterID);
    [CSharpCallLua] public delegate void NotifyInput_Bool2(int _characterID, bool _b1, bool _b2);
    [CSharpCallLua] public delegate void NotifyInput_Int(int _index);

    private NotifyInput_Bool1 luaOnReloadInput;
    private NotifyInput_Bool1 luaOnSwitchAmmoInput;
    private NotifyInput_Bool2 luaOnFireInput;
    private NotifyInput_Int luaOnCharacterSwapped;
    ...
    protected override void OnDestroy()
    {
        ...
        luaOnFireInput = null;
        luaOnReloadInput = null;
        luaOnSwitchAmmoInput = null;
        luaOnCharacterSwapped = null;
        SafeDisposeLuaEnv();
        ClearLuaScriptCache();
    ...
    public void StartGame()
    {
        ...
        // 解耦委托抓取，全是通过事件通道下发给 Lua 的 main -> WeaponManager
        GlobalLuaEnv.Global.Get("OnLuaFireInput", out luaOnFireInput);
        GlobalLuaEnv.Global.Get("OnLuaReloadInput", out luaOnReloadInput);
        GlobalLuaEnv.Global.Get("OnLuaSwitchAmmoInput", out luaOnSwitchAmmoInput);
        GlobalLuaEnv.Global.Get("OnLuaCharacterSwapped", out luaOnCharacterSwapped);
    }

    /// <summary>
    /// 向 Lua 发送极其轻量级的平铺 Hit 数据 (取代原来的 Struct)
    /// </summary>
    public void SendHitMessageFlat(int _instigatorID, int _targetUID, int _targetConfigID, int _targetLevel, int _ammoID, float _hitMultiplier)
    {
        luaOnHitMessage?.Invoke(_instigatorID, _targetUID, _targetConfigID, _targetLevel, _ammoID, _hitMultiplier);
    }

    /// <summary>
    /// 被 C# 发现动作时调用，向 Lua 推送轻量动作事件
    /// </summary>
    public void NotifyFireInput(int _characterID, bool _isHeld, bool _wasPressed)
    {
        luaOnFireInput?.Invoke(_characterID, _isHeld, _wasPressed);
    }

    public void NotifyReloadInput(int _characterID)
    {
        luaOnReloadInput?.Invoke(_characterID);
    }

    public void NotifySwitchAmmoInput(int _characterID)
    {
        luaOnSwitchAmmoInput?.Invoke(_characterID);
    }

    public void NotifyCharacterSwapped(int _newIndex)
    {
        luaOnCharacterSwapped?.Invoke(_newIndex);
    }
