...
public class CameraController : SingleMonoBase<CameraController>
{
    ...
    void Update()
    {
        ...
        TPSCharacterController player = TeamController.Instance.GetCurrentPlayer();
        if (player != null)
        {
            RecoilComponent currentRecoil = player.GetComponent<RecoilComponent>();
            if (currentRecoil != null)
            {
                currentRecoil.ConsumeCameraRecoil(out Vector2 camKick);
                if (camKick != Vector2.zero)
                {
                    // Cinemachine FreeLook 中，m_XAxis 负责左右旋转，m_YAxis (通常设定为 0~1) 负责高低俯仰。
                    // 乘系数根据你的鼠标灵敏度和工程 YAxis Speed 配置可能需轻微调整：
                    freeLookCamera.m_XAxis.Value += camKick.x * 0.1f;
                    freeLookCamera.m_YAxis.Value += camKick.y * 0.005f;
                }
            }
        }
